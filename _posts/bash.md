...
- Cryptography is taking something and encrypting it with a key so that later down the line someone can take that key to decrypt the message
- There are two types of cryptography:
  - Symmetric (use the same key to lock/unlock messages)
...
- Typical key lengths are 128 and 256 bits for `private keys` and 2048 for `public keys`. Private keys don't have to be that large because the same key is used to encrypt/decrypt and that key never leaves or gets transported along the internet

### Encryption algorithms (cipher)
1. Triple DES 
  - Symmetric encryption methods
  - Slowly being phased out for AES
  - As the name implies, is an upgraded version of the DES algorithm
...
3. RSA Security (public-key encryption algorithm) asymmetric algorithm
  - Asymmetric Encryption Algorithm
  - Invented by Ron *R*ivest, Adi *S*hamir, and Leonard *A*dleman, (hence “RSA”)
  - To date, the most widely used asymmetric encryption algorithm.
4. Blowfish (Designed to replace DES) symmetric cipher, free
5. Twofish () free
...
- `Base64` is encoded data, NOT encryption. It's meant to represent binary data in ascii string format. It's VERY easy to encode/decode without keys and you should never store user passwords in this format.
...
- Basically you would use it to ensure that the data remain intact without modification *during transport*.
- More on cryptography [here](https://tryhackme.com/room/cryptographyfordummies)

  ```shell
  # Encode a value
  $ echo Some_data_to_be_converted | base64
  U29tZV9kYXRhX3RvX2JlX2NvbnZlcnRlZAo=

  # Decode a value
  $ echo U29tZV9kYXRhX3RvX2JlX2NvbnZlcnRlZAo= | base64 -d
